% Resolved-rates control for xArm robot
clear;
close all
...
v_max = 0.7; % max end effector linear velocity, m/s
w_max = 0.7; % max end effector angular velocity, rad/s
v_min = 0.3; % min end effector linear velocity, m/s
w_min = 0.3; % min end effector angular velocity, rad/s
...
iter_max = 500; % max number of iterations for each target

% D-H parameters
params = [[0.065 0 0 0 0.112]' ... % d, m
          [0 0.101 0.095 0.053 0]' ... % a, m
          [pi/2 0 0 0 -pi/2]']; % alpha, rad

needle_len = 0; % length of the needle
...
th4 = 0;
th5 = 0;
q = [th1+(pi/2) th2 th3 th4 th5+(pi/2)]'; 
...
for ite_target = 1:size(Tt,3) % loop through all targets
    ...
    for ite_rr = 1:iter_max + 1 % solve for each target
        ...
        frames = xarm_forward_kinematics_tofunmi(q, params, needle_len);
        Tc = frames(:, :, end);
        ...
        J = xArm_Jacobian_Tofunmi(frames);
